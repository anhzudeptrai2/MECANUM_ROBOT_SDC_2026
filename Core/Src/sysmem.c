...
#if defined(__CC_ARM)
  /* ARMCC/Keil: use scatter-loading region symbols instead of GCC linker script symbols.
   * Put heap in RW_IRAM2 so it won't collide with MSP stack (typically in IRAM1).
   */
  extern uint8_t Image$$RW_IRAM2$$ZI$$Limit;
  extern uint8_t Image$$RW_IRAM2$$Limit;
  const uint8_t *max_heap = &Image$$RW_IRAM2$$Limit;

  /* Initialize heap end at first call */
  if (__sbrk_heap_end == NULL)
  {
    __sbrk_heap_end = &Image$$RW_IRAM2$$ZI$$Limit;
  }
#else
